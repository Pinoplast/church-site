...
<?php echo JLayoutHelper::render('joomla.content.addthis', $this->item); ?>
			<?php if (!$this->item->state) : ?>
				<span class="label label-warning"><?php echo JText::_('JUNPUBLISHED'); ?></span>
			<?php endif; ?>
		</div>
		<?php if ($this->item->avatar) : ?>
			<div class="img-polaroid pull-left item-image hidden-phone">
				
					<img style="width:200px;" src="<?php echo SermonspeakerHelperSermonspeaker::makelink($this->item->avatar); ?>">
					...
					<thead><tr>
						...
						if (in_array('serie:hits', $this->columns)) : ?>
							<th class="ss-col ss-hits hidden-phone hidden-tablet">
								<?php if (!$limit) :
									echo JHtml::_('grid.sort', 'JGLOBAL_STATS', 'hits', $listDirn, $listOrder);
								else :
									echo JText::_('JGLOBAL_STATS');
								endif; ?>
							</th>
						<?php endif;
	...
	<?php endif;
		...
		<td class="ss-col ss-hits hidden-phone hidden-tablet">
			...
			<?php if ($item->custom1) : ?>
				<?php echo  "("; ?>
			<?php endif; ?>	
				<span id=ss-started<?php echo $item->id; ?>  ><?php echo $item->custom1; ?></span>
				<?php if ($item->custom1) : ?>
				<?php echo  "/"; ?>
			<?php endif; ?>	
				<span id=ss-complited<?php echo $item->id; ?>  ><?php echo $item->custom2; ?></span>
			<?php if ($item->custom1) : ?>
				<?php echo  ")"; ?>
			<?php endif; ?>
